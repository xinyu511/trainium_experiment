...
for snr in snr_vals:
    ...
    for img, _ in tqdm(test_loader):
        ...
        # Assume class_preds is a 1D tensor of predicted class indices
        class_preds = torch.cat(class_probs)  # Shape: [mc_samples]

        # Convert to one-hot: shape [mc_samples, num_classes]
        one_hot = torch.nn.functional.one_hot(class_preds, num_classes=10).float()

        # Sum one-hot vectors to count class occurrences
        class_counts = one_hot.sum(dim=0)  # Shape: [10]

        # Normalize to get probabilities
        class_probs = class_counts / class_preds.shape[0]
        ...
        errs.append(torch.sum(torch.abs(class_probs.flatten() - pred_probs.flatten())).item())

    with open('cifar10_results_' + str(snr) + '.pkl', 'wb') as f:
        pickle.dump(errs, f)
